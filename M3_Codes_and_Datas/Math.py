...
from Markov_Chain_Algorithm import MarkovChain
    ...
    for i in range(len(array)):
        ...
        for j in range(i + 1, len(array)):
            ...
            for z in range(len(vector1)):
                ...
                C[i][j] += tmp / math.log(1 + N[z])
    dismat = np.zeros((len(array), len(array)))
    for i in range(len(array)):
        vector1 = np.array(array[i])
        for j in range(i + 1, len(array)):
            vector2 = np.array(array[j])
            dist = C[i][j] / math.sqrt(np.linalg.norm(vector1) * np.linalg.norm(vector2))
            dismat[i][j] = dist
            dismat[j][i] = dist
...
def community_maker(dismat,threshold,tags=None):
    ...
    for communities in itertools.islice(comp, max_shown):
        ...
        plt.savefig('Graph_tmp-{}.png'.format(shown_count))
        plt.show()
        shown_count += 1

array = data_reader('user_book_tag.csv')
...
print(dismat)
dismat = analysis(dismat)
# dismat = MarkovChain(dismat)
# save_matrix(dismat)

community_maker(dismat,0.13,tags)
